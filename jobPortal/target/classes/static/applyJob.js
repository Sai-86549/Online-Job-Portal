...
app.controller('myCtrl', function ($scope) {
    ...
    function getJobListData() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            cache: false,
            url: URL + "/joblist.json",
            success: function (response) {
                for (let i in response) {
                    let eventData = response[i];
                    eventData["jobId"] = i;
                    temJobList.push(eventData);
                }
                $scope.jobList = temJobList;
                $scope.$apply();
            }, error: function (error) {
                alert("Something went wrong");
            }
    ...
    $scope.addNewJob = function () {
        if (checkIsNull($("#companyNameId").val()) || checkIsNull($("#postid").val())
            || checkIsNull($("#expectedWorkId").val()) || checkIsNull($scope.jobTypeModel)) {
            alert("Please fill all the required data");
        } else {
            let reqstBody = {
                "companyName": $("#companyNameId").val(),
                "post": $("#postid").val(),
                "workExperienceExpected": $("#expectedWorkId").val(),
                "description": $("#jobDesId").val()
            };
            $.ajax({
                type: 'post',
                contentType: "application/json",
                dataType: 'json',
                cache: false,
                url: URL + "/joblist/" + $scope.jobTypeModel.jobId + "/availableJob.json",
                data: JSON.stringify(reqstBody),
                success: function (response) {
                    getJobListData();
                    clearData();
